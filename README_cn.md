...
## 例子
我们先来看一个很久以前的例子，是这个例子导致了我们开始开发这个库。我们有一个外部的HTTP APP能生产和消费JSON对象，在众多的属性中有一个`titles` 属性:

```js
const sampleTitles = {titles: [{language: "en", text: "Title"},
                               {language: "sv", text: "Rubrik"}]}
```
我们最终想要呈现给用户的是一个富文本编辑器，具有如undo-redo，验证等功能，能跟据给定的JOSN对象来绘制内容。`titles`是模型中很小的一个属性，但是在我们这个例子中我们会只关注这个属性，因为这已经能足够用来展于这些基本概念了。

我们想要的就是一种语言(语法)，能够通过访问titles的`text`属性来做到以下几点
...
再进一步，当更新，插入，删除文本的时候，我们更想把这个JSON对象当成[immutable](#on-immutability)的不可变对象来处理而不是直接更改这个对象。因为对象不可变可以对于支持undo-redo功能有一定的帮助，而且还能减少出bug的概率。
